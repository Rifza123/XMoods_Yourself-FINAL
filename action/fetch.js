...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./action/other/sticker/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
...
exports.modStick = (media, nino, riff, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './action/other/sticker/data.exif', media, '-o', out])
        .on('exit', () => {
            nino.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: riff})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        nino.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: riff })
        fs.unlinkSync(media)
    }
}
